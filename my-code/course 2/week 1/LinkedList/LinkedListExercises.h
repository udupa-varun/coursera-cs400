...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  Node* thru = this->getHeadPtr();
  // traverse List to find appropriate position for new node
  while (thru != nullptr)
  {
    // special case: insertion at front of list
    // create pointer to next node in list
    Node* adjacentNode = thru->next;

    // special case: insertion before first node
    if (thru == this->getHeadPtr())
    {
      // check if size of list is 1, where adjacentNode would be nullptr
    ...
    if (newData > thru->data)
    {
      // special case: current node is last node
      ...
      // regular case: insert between current and adjacent node
      if (newData > thru->data && newData <= adjacentNode->data)
      {
        // create new node in heap memory, assign data
        Node* nodeToInsert = new Node(newData);

        // update pointers
        thru->next = nodeToInsert;
        nodeToInsert->prev = thru;
        nodeToInsert->next = adjacentNode;
        adjacentNode->prev = nodeToInsert;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // edge case: if one is empty, return the other list
  if (left.size() == 0)
  {
    return right;
  }
  ...
  // deplete whichever list has remaining nodes
  while (left.size() > 0)
  {
    merged.pushBack(left.front());
    left.popFront();
...
// template <typename T>
// LinkedList<T> LinkedList<T>::sortedMerge(typename LinkedList<T>::Node* leftHead, typename LinkedList<T>::Node* rightHead)
// {
//   LinkedList<T> merged;
...
//   {
//     merged = leftHead;
//     merged->next = sortedMerge(leftHead->next, rightHead);
//   }
//   else
//   {
//     merged = rightHead;
//     merged->next = sortedMerge(leftHead, rightHead->next);
//   }
// }